...
| ------------- | ---------- | ----------- | ----------- | --------------------- |
| LLM01:2025 Prompt Injection | High | Altering the behavior or output of LLMs through crafted inputs | CIS Control 3: Data Protection | Implement input validation to prevent malicious input that can alter LLM behavior. |
...
| LLM02:2025 Sensitive Information Disclosure | High | Unintentional exposure of sensitive information through LLM outputs | CIS Control 3: Data Protection | Implement data masking and sanitization techniques to prevent the exposure of sensitive information. |
...
| LLM03:2025 Supply Chain Vulnerabilities | Medium | Dependency on third-party components leading to risks like hijacking | CIS Control 2: Inventory and Control of Software Assets | Maintain an updated inventory of all software components used by LLMs. |
...
| LLM04:2025 Data and Model Poisoning | High | Feeding malicious data into an LLM's training set, corrupting its behavior | CIS Control 7: Continuous Vulnerability Management | Regularly assess the quality and integrity of training data to prevent poisoning. |
...
| LLM05:2025 Insecure Output Handling | Medium | Generating harmful content, leading to exploitation or abuse | CIS Control 3: Data Protection | Use output filtering to prevent harmful or sensitive data from being generated. |
...
| LLM06:2025 Excessive Agency | Medium | LLMs have too much autonomy, leading to unauthorized actions | CIS Control 5: Account Management | Apply the principle of least privilege to limit LLM capabilities to only necessary actions. |
...
| LLM07:2025 System Prompt Leakage | Medium | Exposing system prompts or configuration details, leading to breaches | CIS Control 4: Secure Configuration | Conceal system prompts to prevent leakage of sensitive configurations. |
...
| LLM08:2025 Vector and Embedding Weaknesses | Medium | Misuse of vectors and embeddings to manipulate LLM outputs or processes | CIS Control 16: Application Software Security | Secure embedding techniques and apply controls to prevent manipulations. |
...
| LLM09:2025 Misinformation | High | Generating or propagating false information, leading to harmful decisions | CIS Control 13: Network Monitoring and Defense | Monitor and filter misinformation spread via LLM outputs. |
...
| LLM10:2025 Unbounded Consumption | Medium | Resource exhaustion caused by unbounded processing or interactions | CIS Control 4: Secure Configuration | Implement resource constraints to prevent resource exhaustion. |
